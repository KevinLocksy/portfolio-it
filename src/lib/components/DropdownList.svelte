...
<div class="component">
	<div
		...
		role="button"
		use:clickOutside
		on:click_outside={closeDropdownList}
		tabindex="0"
		on:keydown={() => {}}
	>
		<button
			class="options-selected"
			data-option-id={selectedId}
			value={selectedValue}
			on:click={() => toggleDropdownList(open)}
		>
			<img
				class="flag icon"
				src={options[selectedId].logo}
				alt={options[selectedId].value}
				title={options[selectedId].label}
			/>
