<script>
	 ...
	 */
	function flipCard(e) {
		if (e.target.nodeName === nodeType_URL || e.target.parentNode.nodeName === nodeType_URL) return;

		const isTransformEmpty =
			typeof card.style.transform === 'string' && card.style.transform.length === 0;
...
<div
	bind:this={card}
	class="component card animation"
	role="button"
	on:click={flipCard}
	on:keyup
	tabindex="0"
...
<style>
  ...
  */
	...
	.animation:hover {
		animation-name: rotation var(--card-rotation-duration) var(--card-rotation-iteration) linear
			forwards;
		-webkit-animation: rotation var(--card-rotation-duration) var(--card-rotation-iteration) linear
			forwards;
	}
	@keyframes rotation {
		25% {
			transform: rotateY(var(--card-rotation-angle));
		}
		75% {
			transform: rotateY(-var(--card-rotation-angle));
		}
  ...
  */
	.dot {
		...
		width: 5px;
		box-shadow:
			inset 0px 0px 10px 2px var(--dot-shadow-primary),
